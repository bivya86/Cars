...
def insert_data(request):
    ...
    if request.method =='POST' and request.FILES:
        CO = CarForm(request.POST,request.FILES)
        if CO.is_valid():
            CO.save()
...
class CarData(viewsets.ViewSet):
    def list(self,request):
        ADO=Cars.objects.all()
        SJD=CarsMS(ADO,many=True)
        d={'data':SJD.data}
        return render(request,'list.html',d)

    def retrieve(self,request,pk):
        TO=Cars.objects.get(pk=pk)
        SDO=CarsMS(TO)
        return Response(SDO.data)

    def update(self,request,pk):
        SPO=Cars.objects.get(pk=pk)
        SPD=CarsMS(SPO,data=request.data)
        if SPD.is_valid():
            SPD.save()
            return Response({'Updated':'Car is updated'})
        else:
            return Response({'Failed':'Car is Not Updated'})
    
    def partial_update(self,request,pk):
        SPO=Cars.objects.get(pk=pk)
        SPD=CarsMS(SPO,data=request.data,partial=True)
        if SPD.is_valid():
            SPD.save()
            return Response({'Updated':'Car is updated'})
        else:
    ...
    def destroy(self,request,pk):
        ...
        return Response({'Deleted':'Car is deleted'})
